...
tuple<Matrix&,Matrix&,Matrix&,Matrix&,Matrix&,Matrix&,Matrix&,Matrix&,Matrix&,Matrix&,Matrix&> JPL_Eph_DE430(double Mjd_TDB){
	...
	Matrix& r_Earth = Cheb3D(Mjd_TDB, 13.0, Mjd0, Mjd0+16.0, Cx_Earth.extract_vector(13 * j + 1, 13 * j + 13),
						 Cy_Earth.extract_vector(13 * j + 1, 13 * j + 13), Cz_Earth.extract_vector(13 * j + 1, 13 * j + 13)).transpose()*1e3;
	

	for (int j = 0; j < 4; j++) {
		temp(j+1) = 441.0 + 13.0 * j;  
	}
	...
	Matrix& r_Moon = Cheb3D(Mjd_TDB, 13.0, Mjd0, Mjd0+4.0, Cx_Moon.extract_vector(13 * j + 1, 13 * j + 13),
						Cy_Moon.extract_vector(13 * j + 1, 13 * j + 13), Cz_Moon.extract_vector(13 * j + 1, 13 * j + 13)).transpose()*1e3;

	
	for (int j = 0; j < 4; j++) {
		temp(j + 1) = 753.0 + 11.0 * j;
	}
	...
	Matrix& r_Sun = Cheb3D(Mjd_TDB, 11.0, Mjd0, Mjd0+16.0, Cx_Sun.extract_vector(11 * j + 1, 11 * j + 11),
	...
	Matrix& r_Mercury = Cheb3D(Mjd_TDB, 14.0, Mjd0, Mjd0+8.0, Cx_Mercury.extract_vector(14 * j + 1, 14 * j + 14),
						   Cy_Mercury.extract_vector(14 * j + 1, 14 * j + 14), Cz_Mercury.extract_vector(14 * j + 1, 14 * j + 14)).transpose()*1e3;

	
	for (int j = 0; j < 4; j++) {
		temp(j + 1) = 171.0 + 10.0 * j;
	}
	...
	Matrix & r_Venus = Cheb3D(Mjd_TDB, 10.0, Mjd0, Mjd0+16.0, Cx_Venus.extract_vector(10 * j + 1, 10 * j + 10),
						 Cy_Venus.extract_vector(10 * j + 1, 10 * j + 10),Cz_Venus.extract_vector(10 * j + 1, 10 * j + 10)).transpose()*1e3;

	
	for (int j = 0; j < 4; j++) {
		temp(j + 1) = 309.0 + 11.0 * j;
	}
	...
	j=0;
	Mjd0 = t1;
	Matrix& r_Mars = Cheb3D(Mjd_TDB, 11.0, Mjd0, Mjd0+32.0, Cx_Mars.extract_vector(11 * j + 1, 11 * j + 11),
						Cy_Mars.extract_vector(11 * j + 1, 11 * j + 11), Cz_Mars.extract_vector(11 * j + 1, 11 * j + 11)).transpose()*1e3;

	
	for(int j = 0; j < 4; j++) {
		temp(j + 1) = 342.0 + 8.0 * j;
	}
	...
	j=0;
	Mjd0 = t1;
	Matrix& r_Jupiter = Cheb3D(Mjd_TDB, 8.0, Mjd0, Mjd0+32.0, Cx_Jupiter.extract_vector(8 * j + 1, 8 * j + 8),
						   Cy_Jupiter.extract_vector(8 * j + 1, 8 * j + 8), Cz_Jupiter.extract_vector(8 * j + 1, 8 * j + 8)).transpose()*1e3;

	
	for (int j = 0; j < 4; j++) {
		temp(j + 1) = 366.0 + 7.0 * j;
	}
	...
	j=0;
	Mjd0 = t1;
	Matrix& r_Saturn = Cheb3D(Mjd_TDB, 7.0, Mjd0, Mjd0+32.0, Cx_Saturn.extract_vector(7*j+1, 7*j+7),
						  Cy_Saturn.extract_vector(7*j+1, 7*j+7), Cz_Saturn.extract_vector(7*j+1, 7*j+7)).transpose()*1e3;
	
	
	
	for(int j = 0; j < 4; j++) {
		temp(j + 1) = 387.0 + 6.0 * j;
	}
	...
	j=0;
	Mjd0 = t1;
	Matrix& r_Uranus = Cheb3D(Mjd_TDB, 6.0, Mjd0, Mjd0+32.0, Cx_Uranus.extract_vector(6 * j + 1, 6 * j + 6),
						  Cy_Uranus.extract_vector(6 * j + 1, 6 * j + 6), Cz_Uranus.extract_vector(6 * j + 1, 6 * j + 6)).transpose()*1e3;

	
	for(int j = 0; j < 4; j++) {
		temp(j + 1) = 405.0 + 6.0 * j;
	}
	...
	j=0;
	Mjd0 = t1;
	Matrix& r_Neptune = Cheb3D(Mjd_TDB, 6.0, Mjd0, Mjd0+32.0, Cx_Neptune.extract_vector(6 * j + 1, 6 * j + 6),
						   Cy_Neptune.extract_vector(6 * j + 1, 6 * j + 6), Cz_Neptune.extract_vector(6 * j + 1, 6 * j + 6)).transpose()*1e3;

	
	for (int j = 0; j < 4; j++) {
		temp(j + 1) = 423.0 + 6.0 * j;
	}
	...
	j=0;
	Mjd0 = t1;
	Matrix& r_Pluto = Cheb3D(Mjd_TDB, 6.0, Mjd0, Mjd0+32.0, Cx_Pluto.extract_vector(6 * j + 1, 6 * j + 6),
						 Cy_Pluto.extract_vector(6 * j + 1, 6 * j + 6), Cz_Pluto.extract_vector(6 * j + 1, 6 * j + 6)).transpose()*1e3;

	
	for (int j = 0; j < 4; j++) {
		temp(j + 1) = 819.0 + 10.0 * j;
	}
	...
	Matrix& Nutations = Cheb3D(Mjd_TDB, 10.0, Mjd0, Mjd0+8.0, Cx_Nutations.extract_vector(10*j + 1, 10*j + 10),
					   Cy_Nutations.extract_vector(10*j + 1, 10*j + 10),zeros(10,1)).transpose();
    

	for (int j = 0; j < 4; j++) {
		temp(j + 1) = 899.0 + 10.0 * j;
	}
	...
	Matrix& Librations = Cheb3D(Mjd_TDB, 10.0, Mjd0, Mjd0+8.0, Cx_Librations.extract_vector(10*j+1, 10*j+10),
