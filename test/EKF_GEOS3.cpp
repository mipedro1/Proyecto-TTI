...
int main(){
	...
	DE430Coeff(2285,1020); // f=2285 c=1020
	...
	double sigma_range,sigma_az,sigma_el,lat,lon,alt,Mjd1,Mjd2,Mjd3,theta,Dist;
	...
	Matrix& Y0_apr = zeros(2,3);
	Y0_apr(1,1)=r2(1); Y0_apr(1,2)=r2(2); Y0_apr(1,3)=r2(3);
	Y0_apr(2,1)=v2(1); Y0_apr(2,2)=v2(2); Y0_apr(2,3)=v2(3);
	...
	for (int i=1;i<=nobs;i++){   
		...
		// Measurement update
		auto [K, Y, P] = MeasUpdate ( Y, obs(i,2), Azim, sigma_az, dAdY, P, 6 );
		...
		// Measurement update
		auto [K, Y, P] = MeasUpdate ( Y, obs(i,3), Elev, sigma_el, dEdY, P, 6 );
		...
		// Measurement update
		auto [K, Y, P] = MeasUpdate ( Y, obs(i,4), Dist, sigma_range, dDdY, P, 6 );
	}
